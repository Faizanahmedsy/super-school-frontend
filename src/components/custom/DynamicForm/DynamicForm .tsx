...
import { Form, Input, Select, Button, Checkbox, DatePicker, FormInstance } from 'antd';
...
const DynamicForm: React.FC<DynamicFormProps> = ({ formConfig, onFinish }) => {
  ...
  const renderFormItem = (item: FormItemConfig) => {
    ...
    switch (item.type) {
      ...
      case 'select':
        return (
          <UISelect placeholder={item.placeholder || 'Select value'}>
  ...
  return (
    <Form form={form} onFinish={onFinish as unknown as () => void} layout="vertical" className="space-y-6">
