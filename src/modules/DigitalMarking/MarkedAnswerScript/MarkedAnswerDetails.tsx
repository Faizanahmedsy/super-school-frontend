...
export default function MarkedAnswerDetails() {
  ...
  const { data: getSubjectWiseStudent } = useAssessmentWiseStudentstList({
    ...pageQuery,
    filter_by_upload: true,
    sheet_update: true,
    assessment_subject: subjectDetailsId.subjectdetailsid?.id,
    grade_class: subjectDetailsId.gradeclass?.id ? subjectDetailsId.gradeclass?.id : undefined,
  });

  const { data: assessmentsDetails, isLoading } = useAssessmentDetails(params?.id, {
    grade_class: subjectDetailsId.gradeclass?.id,
    school_id: school_id ? school_id : undefined,
  });
  ...
  const columns: any = [
    ...
    },
    {
      ...
      cell: () => {
        const mark_percentage = getSubjectWiseStudent?.results[0]?.mark_percentage;
        return (
    ...
    {
      accessorKey: 'actual_mark',
      header: 'Marks',
      cell: () => {
        const paper_marks = getSubjectWiseStudent?.results[0]?.paper_marks;
        const obtained_mark = getSubjectWiseStudent?.results[0]?.obtained_mark;
        return (
          <div className="w-20">
            <p>{`${obtained_mark ? obtained_mark : '-'}/${paper_marks ? paper_marks : '-'}`}</p>
          </div>
        );
      },
    },
...

    {
      ...
      header: 'Answersheet',
      cell: () => {
        const data = getSubjectWiseStudent?.results[0];
    ...
    {
      id: 'actions',
      header: user?.role_name === ROLE_NAME.STUDENT ? 'Review Answer Sheet' : 'Actions',
            ...
            {user?.role_name === ROLE_NAME.STUDENT ? (
              <div className="flex space-x-2">
                <Button
                  ...
                  onClick={() => {
                    const componentName = 'graded-papers';
                    navigate(`/marked-answerscript/instant-grades/review/${getSubjectWiseStudent?.results[0]?.id}`, {
                      state: {
                        componentName,
                        assessment_subject: row.original?.id,
                        grade_class: row.original?.grade_class_ids ? row.original?.grade_class_ids : undefined,
                      },
                    });
                  }}
                >
  ...
  if (user?.role_name === ROLE_NAME.STUDENT) {
    removeColumns = {
      columns,
      restrictedKeys: ['grade_class__name', 'grade__grade_number', 'ocr_status', 'status'],
      additionalColumns: [],
      roleName: ROLE_NAME.STUDENT,
    };
  } else if (user?.role_name === ROLE_NAME.ADMIN) {
    removeColumns = {
      columns,
      restrictedKeys: ['achievements_level', 'actual_mark', 'answer_sheet'],
      additionalColumns: [],
      roleName: ROLE_NAME.ADMIN,
    };
  } else if (user?.role_name === ROLE_NAME.TEACHER) {
    removeColumns = {
      columns,
      restrictedKeys: ['achievements_level', 'actual_mark', 'answer_sheet'],
      additionalColumns: [],
  ...
  } else if (user?.role_name === ROLE_NAME.SUPER_ADMIN) {
    removeColumns = {
      columns,
      restrictedKeys: ['achievements_level', 'actual_mark', 'answer_sheet'],
      additionalColumns: [],
  ...
  const handleDetails = (id: number, gradeId: number) => {
    navigate(`/marked-answerscript/assessment/subject-details/${id}?grade_class=${gradeId}`);
  };
        ...
        <PageTitle
          breadcrumbs={[
            { label: 'Graded Papers', href: '/marked-answerscript' },
            {
              ...
              href: `/marked-answerscript/assessment/details/${subjectDetailsId.subjectdetailsid?.id}`,
            },
          ]}
        >
          Assessment Details
