  ...
  'inline-flex items-center justify-center whitespace-nowrap rounded-xl text-sm font-semibold ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 uppercase tracking-wide',
  {
    variants: {
      variant: {
        ...
        secondaryNormal: 'bg-yellow-400 text-black hover:bg-yellow-400/90 border-yellow-400 ',
        danger: 'bg-red-200 text-red-800 hover:bg-red-200/90 border-red-400 border-b-4 active:border-b-0',
        dangerNormal: 'bg-red-400 text-black hover:bg-red-400/90 border-red-400 ',
        ghost: 'hover:bg-accent hover:text-accent-foreground',
        link: 'text-primary underline-offset-4 hover:underline',
        company: 'bg-company text-company-foreground hover:bg-company/90 border-b-4 border-teal-700 active:border-b-0',
        super:
...
const PlayfulButton = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    return <Comp className={cn(PlayfulButtonVariants({ variant, size, className }))} ref={ref} {...props} />;
  }
);
